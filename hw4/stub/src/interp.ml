...
and
  ...
  | Proj(e,id) ->
    (eval_expr e) >>= fields_of_recordVal >>= fun n1 -> 
    (let rec aux n1 = 
      match n1 with
    | (ids, (bool, expval))::t -> if (ids == id) then
    (return expval >>= int_of_refVal >>= Store.deref g_store)
    else (aux t)
    | [] -> error "Field not found" in (aux n1))
  | SetField(e1,id,e2) ->
    (eval_expr e1) >>= fields_of_recordVal >>= fun n1 -> 
    (let rec aux n1 = 
      match n1 with
    | (ids, (bool, expval))::t -> if (ids == id) then
    (return expval >>= int_of_refVal >>= fun l -> eval_expr e2 >>= Store.set_ref g_store l >>= fun _ -> return UnitVal)
    else (aux t)
    | [] -> error "Field not found" in (aux n1))
  | IsNumber(e) ->
    eval_expr e >>= is_numVal >>= fun r -> return (BoolVal (r))
  | Unit -> return UnitVal
  | Debug(_e) ->
